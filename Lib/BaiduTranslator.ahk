...
{
  init(edgePath:="Edge\msedge.exe", profilePath:="baidu_translate_profile", debugPort:=9890, mode:="sync", timeout:=30)
  {
    ...
    if (Edge.FindInstances().HasKey(debugPort))
      ...
      EdgeInst := new Edge(profilePath,, "--headless", edgePath, debugPort)
  ...
  _multiLanguage()
  {
    ...
    {
      l.1 := "Please set the Edge.exe path first!"
